...
this repository specifies configuration for nixosdeploy script.

## Usage

NixDeploy consist of NixOS configuration module and `nixosdeploy` tool itself.
...
The next step is to actually run `nixosdeploy`. You have few options how to run
it, you can either run it directly from this flake with `nix run
gitlab:cynerd/nixosdeploy` or you can install it as package to your system, but
...
Now you can read the help of the `nixosdeploy` with `nix run . -- -h`.
